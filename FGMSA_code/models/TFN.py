...
class TFN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.fusion_subnet = FusionSubNet(self.post_fusion_dim, self.output_dim, self.fusion_prob)
    ...
    def forward(self, text_cutx, audio_cutx, video_cutx):
        ...
        for i in range(batch_size):
            ...
            fusion_tensor = torch.bmm(_audio_cuth.unsqueeze(2), _video_cuth.unsqueeze(1))

            fusion_tensor = fusion_tensor.view(-1, (self.audio_hidden + 1) * (self.video_hidden + 1), 1)
            fusion_tensor = torch.bmm(fusion_tensor, _text_cuth.unsqueeze(1)).view(seq_size, -1)
            ...
            fusion = self.fusion_subnet(post_fusion_y_1, output_fusion)
